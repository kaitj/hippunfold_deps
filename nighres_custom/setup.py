...
INCLUDES = {
    ...
    "linux": ["%(linux)s/include" % (JDK), "%(linux)s/include/linux" % (JDK)],
    "sunos5": [
        "%(sunos5)s/include" % (JDK),
        "%(sunos5)s/include/solaris" % (JDK),
    ],
    "win32": ["%(win32)s/include" % (JDK), "%(win32)s/include/win32" % (JDK)],
    "mingw32": [
        "%(mingw32)s/include" % (JDK),
        "%(mingw32)s/include/win32" % (JDK),
    ],
    "freebsd7": [
        "%(freebsd7)s/include" % (JDK),
...
LFLAGS = {
    ...
    "darwin/adoptopenjdk": [
        "-L%(darwin)s/lib" % (JDK),
        "-ljava",
        "-L%(darwin)s/lib/server" % (JDK),
        "-ljvm",
        "-Wl,-rpath",
        "-Wl,%(darwin)s/lib" % (JDK),
        "-Wl,-rpath",
        "-Wl,%(darwin)s/lib/server" % (JDK),
        "-mmacosx-version-min=10.9",
    ],
    "darwin/home": [
        "-L%(darwin)s/jre/lib" % (JDK),
        "-ljava",
        "-L%(darwin)s/jre/lib/server" % (JDK),
        "-ljvm",
        "-Wl,-rpath",
        "-Wl,%(darwin)s/jre/lib" % (JDK),
        "-Wl,-rpath",
        "-Wl,%(darwin)s/jre/lib/server" % (JDK),
        "-mmacosx-version-min=10.9",
    ],
    "ipod": [
        "-ljvm",
        "-lpython%s.%s" % (sys.version_info[0:2]),
        "-L/usr/lib/gcc/arm-apple-darwin9/4.0.1",
    ],
    "linux/i386": [
        "-L%(linux)s/jre/lib/i386" % (JDK),
        "-ljava",
        "-L%(linux)s/jre/lib/i386/client" % (JDK),
        "-ljvm",
        "-Wl,-rpath=%(linux)s/jre/lib/i386:%(linux)s/jre/lib/i386/client"
        % (JDK),
    ],
    "linux/i686": [
        "-L%(linux)s/jre/lib/i386" % (JDK),
    ...
    "sunos5": [
        "-L%(sunos5)s/jre/lib/i386" % (JDK),
        "-ljava",
        "-L%(sunos5)s/jre/lib/i386/client" % (JDK),
        "-ljvm",
        "-R%(sunos5)s/jre/lib/i386:%(sunos5)s/jre/lib/i386/client" % (JDK),
    ],
    "win32": [
        "/DLL",
        "/LIBPATH:%(win32)s/lib" % (JDK),
        "Ws2_32.lib",
        "jvm.lib",
    ],
    "mingw32": ["-L%(mingw32)s/lib" % (JDK), "-ljvm"],
    "freebsd7": [
        "-L%(freebsd7)s/jre/lib/i386" % (JDK),
        "-ljava",
        "-lverify",
        "-L%(freebsd7)s/jre/lib/i386/client" % (JDK),
        "-ljvm",
...
IMPLIB_LFLAGS = {"win32": ["/IMPLIB:%s"], "mingw32": ["-Wl,--out-implib,%s"]}

if platform == "linux":
    LFLAGS["linux"] = LFLAGS["linux/%s" % (machine)]
elif platform == "darwin":
    if JAVAHOME is not None:
        if "adoptopenjdk" in JAVAHOME:
...
JAVAC = {
    "darwin": ["javac", "-source", "1.8", "-target", "1.8"],
...
try:
    ...
    if with_modern_setuptools and "NO_SHARED" not in os.environ:
        ...
        if enable_shared:
            print("enable_shared is ON")
        else:
            print("enable_shared is OFF")
...
def main(debug):
    ...
    jcc_build_py.config_file = os.path.join(
        os.path.dirname(os.path.abspath(__file__)),
        "jcc%s" % (py_version_suffix),
        "config.py",
    )
    ...
    if with_setuptools and enable_shared:
        ...
        if platform in ("darwin", "ipod"):
            ...
            kwds["extra_link_args"] = lflags + [
                implib_flags
                % (os.path.join("jcc%s" % (py_version_suffix), jcclib))
            ]
